...
**상황 설정**: 게시글의 좋아요 카운터가 0이다. 두 사용자가 **동시에** ❤️ 를 누른다. 각자 "지금 값을 읽고 → +1 → 저장" 하는 흐름이므로, 결과는 **2**가 되어야 한다.
...
시간↓      T1 (사용자 A)            T2 (사용자 B)
   ...
   └───────────────────┘ ┌────────┴──────────┐
                         ...
                         └───────────────────┘
             ▼                    ▼
          결과: 카운터 = 1   (기대: 2)  ← 한 번의 ❤️ 가 증발  ✗
```

두 클라이언트가 같은 행을 **read → modify → write** 할 때, 둘 다 같은 "0"을 출발점으로 삼아 각자 "1"을 쓰면 한쪽 증가가 **묵살(clobber)** 된다. 이것이 **갱신 손실(lost update)** 이다. ORM(JPA)으로 엔티티를 읽어 `+1` 후 `save()`하면 기본적으로 이 꼴이 난다 — 책 p.243이 "ORM은 안전하지 않은 read-modify-write를 쓰기 쉽게 만든다"고 경고하는 이유.
...
**이 장에서 가장 중요한 예시.** 상황 설정: 한 교대근무(shift)에 당직 의사가 **2명(Alice, Bob)** 있다. 병원 규칙(불변식): *"이 shift에는 항상 1명 이상 당직 의사가 있어야 한다."* Alice와 Bob이 모두 아파서 **동시에** 당직을 포기한다 — 각자 "다른 한 명이 남으니 괜찮아"라고 판단하며.
...
**상황 설정**: 회의실 A, 12:00~13:00 예약을 두 사람이 **동시에** 시도한다. 규칙(불변식): *"같은 방, 겹치는 시간엔 예약이 2개일 수 없다."* 예약 로직은 (1) 겹치는 기존 예약을 검색하고 (2) 없으면 INSERT 하는 흐름이다.
